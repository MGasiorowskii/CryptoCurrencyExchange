...
def buy_now(context: dict, request: HttpRequest, actual_price: float, token_pk: int) -> dict:
    ...
    if form.is_valid() and transaction_price <= user_usdt_wallet.quantity:
        ...
        if amount_buyer <= exchange_token_wallet.quantity:
            ...
            messages.success(request, f"You bought  {amount_buyer} {token.symbol}  for  {transaction_price} USDT")
        else:
